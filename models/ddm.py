...
class EMAHelper(object):
    ...
    def update(self, module):
        ...
        for name, param in module.named_parameters():
            if param.requires_grad:
                self.shadow[name].data = (1. - self.mu) * param.data + self.mu * self.shadow[name].data.to(param.data.device)
...
class DenoisingDiffusion(object):
    ...
    def train(self, DATASET):
        ...
        if os.path.isfile(self.config.training.resume + '.pth'):
            self.load_ddm_ckpt(self.config.training.resume + '.pth')
